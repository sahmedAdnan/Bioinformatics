...
def generate_input(n):
    ...
    gap_open = 5
    gap_extend = 5 
    return sequence1, sequence2, substitution_matrix, gap_open, gap_extend
...
for n in input_sizes:
    sequence1, sequence2, substitution_matrix, gap_open, gap_extend = generate_input(n)
    start_time = time.time()
    global_affine_alignment(sequence1, sequence2, substitution_matrix, gap_open, gap_extend, backtracking=False)
    end_time = time.time()
...
plt.title('Measured (affine) vs. Theoretical Runtime', fontsize=14)
fig.tight_layout()  
plt.show()